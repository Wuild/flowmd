/**
 * Ordered list plugin for the editor
 */
...
/**
 * Plugin that adds an ordered list button to the toolbar
 */
export class OrderedList extends ToolbarBase {
  /**
  ...
  /**
   * Keymap for ordered list
   */
  get keymap() {
    return {
      'Shift-Ctrl-9': wrapInList(this.editor.schema.nodes.ordered_list),
  ...
  get buttonDefinition(): ToolbarButtonDefinition {
    return {
      name: 'ordered_list',
      title: 'Ordered List',
      icon: createIconSync('ordered_list'),
      isActive: (view: EditorView) => {
        ...
        for (let i = $from.depth; i >= 0; i--) {
          if ($from.node(i).type === this.editor.schema.nodes.ordered_list) {
            return true;
          }
        }
        return false;
      ...
      action: (view: EditorView) => {
        wrapInList(this.editor.schema.nodes.ordered_list)(view.state, view.dispatch);
        view.focus();
      }
    };
  }
